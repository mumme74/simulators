...
class MulDivFractionConstants extends FractionRuleBase {
  constructor() {
    ...
    `, precedence.MulAndDiv, ['mul','div']);// Object.keys(operCb));
  ...
  */
  ...
  _operCb(lNode, rNode, node, args) {
    ...
    const newFrac = lNode.value[node.type](rNode.value);
    this._doChange(node, args, newFrac, lNode, rNode, )

    lNode.value[node.type](rNode.value);
  }
  ...
  /*_div(lChild, rChild, node, args) { // eslint-disable-line unused-variable
    ...
    const newParent = new SolveTreeNode(
      divNode.astNode, 'mul', null, divNode.depth,
      divNode.expression, divNode.equation);

    const newRValue = new Fraction(
      rChild.value.denominator, rChild.value.numerator);
    const newRChild = super._replaceFraction(args, newRValue, rChild, "Flip denominator and numerator", 'div');
  ...
  _expand(factNode1, args) {
    ...
    this._simplifyFractionsTogether(fractions,
      (idx, frac)=>{
        ...
        if (oldFrac.value.denominator.value() != frac.denominator.value())
          this._replaceFraction(args, frac, oldFrac, "Expand to common denominator", 'expand');
    });
