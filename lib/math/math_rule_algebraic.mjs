...
import { ValueBase, Variable } from "./math_values.mjs";
...
RIGHT                                         LEFT
  ...
  */
  run(node, args) {
    ...
    let addSubs = this._nodesOfTypeInExpr(node, ['add','sub']);
    let terms = addSubs.map(n=>this._childrenOf(n, [Variable]));

    if (!terms.length) return;

    terms = terms.reduce((arr, vlu)=>{
      const o = arr.find(id=>id.value?.letter === vlu.value?.letter && id !== vlu);
      if (!o) return arr;
    ...
    const clones = factors.map(f=>f.clone(true));
