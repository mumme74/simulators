 ...
 */
 export class SolveTreeNode extends AstTreeNode {
   ...
   * @param {ValueBase | null} value The value this node has, if not a value node it has no value
  ...
  runStep(args, prec) {
    ...
    if (this.isChildOf(args.engine.solveTreeRoot))
      args.engine.runRulesOnNode(this, prec, args);
  }

  asObject() {
    const superAsObject = super.asObject();
    if (this.value instanceof ValueBase)
  ...
  toString() {
    const ops = ValueBase.operatorToStr; /*{
      eq:'=',sub:'-',add:'+',div:'/',root:'√',
      nthRoot:'√',exp:'^'};*/
