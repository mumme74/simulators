 ...
 export class ValueBase {
  ...
  _operTbl(operCb, operator) {
    ...
    return {
      ...
      fraction: (rsideVlu)=>{
        ...
        return lFrac[operator](rsideVlu);
      },
      variable: (rsideVlu)=>{
        if (operator !== 'mul')
  ...
  add(rsideVlu) {
    const operTbl = this._operTbl(
      ()=>this._value + rsideVlu._value, 'add');
    return ValueBase.create(operTbl[rsideVlu.type](rsideVlu));
  }
  sub(rsideVlu) {
    const operTbl = this._operTbl(
      ()=>this._value - rsideVlu._value, 'sub');
    return ValueBase.create(operTbl[rsideVlu.type](rsideVlu));
  }
  mul(rsideVlu) {
    const operTbl = this._operTbl(
      ()=>this._value * rsideVlu._value, 'mul');
    return ValueBase.create(operTbl[rsideVlu.type](rsideVlu));
  }
  div(rsideVlu) {
    const operTbl = this._operTbl(
      ()=>this._value / rsideVlu._value, 'div');
    return ValueBase.create(operTbl[rsideVlu.type](rsideVlu));
  }
  exp(rsideVlu) {
    const operTbl = this._operTbl(
      ()=>Math.pow(this._value, rsideVlu._value), 'exp');
    return ValueBase.create(operTbl[rsideVlu.type](rsideVlu));
  }
  root() {
    const operTbl = this._operTbl(
      ()=>Math.sqrt(this._value), 'root');
    return ValueBase.create(operTbl[this.type]());
  }
  nthRoot(rsideVlu) {
    const operTbl = this._operTbl(
      ()=>Math.pow(rsideVlu._value, (1 / this._value)), 'nthRoot');
...
export class Variable extends ValueBase {
  ...
  root() {
    const operTbl = this._operTbl(
      ()=>Math.root(this._value), 'root');
    return ValueBase.create(operTbl[rsideVlu.type]());
  }
  nthRoot(rsideVlu) {
    const operTbl = this._operTbl(
      ()=>Math.pow(rside._value, (1 / this._value)), 'nthRoot');
...
export class Fraction extends ValueBase {
  ...
  expandTo(newDenominator) {
    ...
    if (newDenominator.type === 'integer') {
      if (this.denominator.type === 'integer') {
        const factor = ValueBase.create(newDenominator._value / this.denominator._value);
        this.numerator = this.numerator.mul(factor);
        this.denominator = this.denominator.mul(factor);
      } else {
      ...
      if (this.denominator.type === 'integer') {
        ...
        newDenom.coeficient *= this.denominator._value;
        this.denominator = newDenom;
      } else {
        this.denominator.mul(newDenominator);
      }
    } else {
  ...
  div(rsideVlu) {
    ...
    res.numerator = res.numerator.mul(rsideVlu.denominator);
    res.denominator = res.denominator.mul(rsideVlu.numerator);
    return res;
  }
  ...
  static leastCommonDenominator(fractions) {
    ...
    const checkBounds = (fracs) =>{
      ...
      resDenom = fractions.reduce((o, vlu)=>{
    ...
    if (resDenom.big === resDenom.small) return resDenom.big;
    ...
    const primeTest = (fracs) => {
      do {
        for(const prime of [2,3,5,7]) {// test modulo of prime 2, 3, 5, 7
          let frac;
          usePrime = prime;
          for (let i = 0; i < fracs.length; ++i) {
            frac = fracs[i];
            if ((frac.denominator.value() % prime) !== 0 ||
                (frac.numerator.value() % prime) !== 0 ||
                frac.numerator.value() < prime ||
                frac.denominator.value() < prime)
            {
              usePrime = 0;
              break;
            }
  ...
  static shinkTogether(fractions) {
    ...
    const res = fractions.map(f=>
      f.denominator.value() > leastCommon ?
        f.shrink(leastCommon) :f.expandTo(leastCommon)
    );
    return res;
  }
}
