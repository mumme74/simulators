...
import { Value } from "./math_values.mjs";
...
class ChangeSignOnTerms extends MathRuleBase {
  constructor() {
    super(`
  ...
  run(node, args) {
    ...
    for (const term of terms) {
      if (term.right?.type === 'signed') {
        if (node.type === 'add')
          this._swapNode(term, "Uneven signs becomes '-'", opposite, args);
        else if (term.right?.type)
          this._swapNode(term, "Equal signs becomes '+'", opposite, args);
      }
    }
  }
