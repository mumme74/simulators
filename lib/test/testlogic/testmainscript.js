...
const insertTestLink = function(node, parts, linkText) {
  ...
  a.href = '#';
  let hash = parts.files ? parts.files.map(f=>`file=${encodeURI(f)}`).join('&') : '';
  if (parts.suites)
    hash += "&" + (parts.suites ? parts.suites.map(s=>`suite=${encodeURI(s)}`).join('&') : '');
  if (parts.describes)
    hash += "&" + (parts.describes ? parts.describes.map(s=>`describe=${encodeURI(s)}`).join('&') : '');
...
const insertBreak = function(node) {
  if (!testResultNode) return;
...
class Suite {
  ...
  runSuite(filterInDescribes) {
    currSuite = this;

    this.filterInDescribes = filterInDescribes;
  ...
  describe(desc, fn) {
    if (this.filterInDescribes.length &&
        this.filterInDescribes.indexOf(desc) < 0)
      return;

    for (const beforeAll of this._beforeAlls)
  ...
  displayResult() {
        ...
        for(const stat of this._stats) {
            ...
            if (this.fileName) {
              insertBreak(groupNode);
              insertTestLink(groupNode, {files:[this.fileName],suites:[this.suiteName]}, `filter to suite: '${this.suiteName}'`);
              insertBreak(groupNode);
              insertTestLink(groupNode, {files:[this.fileName],suites:[this.suiteName],describes:[stat.name]}, `filter to describe: '${stat.name}'`);
            }
...
globalThis.runTestsInBrowser = (resultDomNode, allFiles) => {
  ...
  const filterInDescribes = filterInParts
        .filter(part=>part.substring(0,9)==='describe=')
        .map(s=>decodeURI(s.substring(9)));
  ...
  document.addEventListener("DOMContentLoaded", async ()=>{
    globalThis.setTestResultToHtml(resultDomNode);
    await globalThis.loadAndRun({allFiles, pathPrefix:'../', filterInFiles, filterInSuites, filterInDescribes});
  });
}
globalThis.runTestsInNode = async (allFiles, filterInFiles=[],filterInSuites=[],filterInDescribes=[])=>{
  await globalThis.loadAndRun({allFiles, pathPrefix:'../',filterInFiles, filterInSuites, filterInDescribes});
}
 ...
 */
globalThis.loadAndRun = async ({
  allFiles, pathPrefix='../', filterInFiles,
  filterInSuites, filterInDescribes
}) => {
  ...
  for (const suite of suitesToRun) {
    ...
    suite.runSuite.apply(suite, [filterInDescribes]);

    for(const afterAllTest of afterAllTests)
      afterAllTest.apply(this);
  }

  globalThis.showTestsResults(suitesToRun);
}
