...
(async () => {
  ...
  console.log(`http://localhost:${s.server.port}/lib/test/`);
  await page.goto(`http://localhost:${s.server.port}/lib/test/`);
  await page.waitForNavigation({
    timeout: 10000, waitUntil:"networkidle0"
  });
  await page.screenshot({ path: resultDir + '/testresult.png' });

  fs.writeFile(resultDir + "/test_result.json", JSON.stringify(result||""), async (err)=>{
    await browser.close();
    s.server.close();
    if (err)
      console.log(err);
    if (process.exitCode === 0 && (!result || result.failedTests !== 0 || err))
      process.exitCode = 1;
  })
