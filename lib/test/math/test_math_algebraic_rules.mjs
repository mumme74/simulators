...
registerTestSuite("test Math engine algebraic add/subtract", ()=>{
  ...
  describe("Should test polynomial expr -b+2a+3ab-4a-2ab+3b", ()=>{
    const math = new MathEngine('-b+2a+3ab-4a-2ab+3b');
    const steps = [];
    ...
    it("Should test step1, xx-xx+3ab-2ab == 2b-2a+ab", ()=>{
      ...
      expect(step.changes[2]?.resultNode?.parent===step.tree).toBe(false);
...
registerTestSuite("test Math engine algebraic mul/div", ()=>{
  describe("Should multiply 1a*2a", ()=>{
    const math = new MathEngine('1a*2a');
    const steps = [];
    it("test step1", ()=>{
      const step = math.solveNextStep();
      ...
      expect(step.changes[0]?.resultNode?.toString()).toBe('2a^2');
      steps.push(step);
    });
    it("Should not have a step2", ()=>{
      const step = math.solveNextStep();
    ...
    it("should have result", ()=>{
      expect(steps[0].tree.toString()).toBe('2a^2');
    });
  });
  /*
  describe("Should multiply 2ab*2a", ()=>{
    const math = new MathEngine('2ab*2a');
    const steps = [];
    it("test step1", ()=>{
      const step = math.solveNextStep();
      expect(step.changes.length).toBe(1);
      expect(step.changes[0]?.inputNodes.length).toBe(2);
      expect(step.changes[0]?.inputNodes[0]?.toString()).toBe('2ab');
      expect(step.changes[0]?.inputNodes[1]?.toString()).toBe('2a');
      expect(step.changes[0]?.operator).toBe('mul');
      expect(step.changes[0]?.resultNode?.toString()).toBe('2a^2b');
      steps.push(step);
    });
    it("Should not have a step2", ()=>{
      const step = math.solveNextStep();
    ...
    it("should have result", ()=>{
      expect(steps[0].tree.toString()).toBe('2a^2b');
    });
  });
  */
});