...
registerTestSuite("test AST node action", ()=>{
  ...
  describe("Test AstTreeNode child manipulation", ()=>{
    let root, left, right, leftRight,
        leftLeft, rightLeft, rightRight;

    beforeEach(()=>{
      root = new AstTreeNode(null, null, 'root'),
      left = new AstTreeNode(null, null, 'left'),
      right = new AstTreeNode(null, null, 'right'),
      leftRight = new AstTreeNode(null, null, 'leftRight'),
      leftLeft  = new AstTreeNode(null, null, 'leftLeft'),
      rightLeft = new AstTreeNode(null, null, 'rightLeft'),
      rightRight = new AstTreeNode(null, null, 'rightRight');
      root.addLeft(left); root.addRight(right);
      left.addLeft(leftLeft); left.addRight(leftRight);
      right.addLeft(rightLeft); right.addRight(rightRight);
    });

    it("Should test addLeft", ()=>{
      root = new AstTreeNode(null, null, 'root'),
      left = new AstTreeNode(null, null, 'left');
      root.addLeft(left);
      ...
      expect(left.left).toBe(undefined);
      expect(left.right).toBe(undefined);
    });

    it("Should test addRight", ()=>{
      root = new AstTreeNode(null, null, 'root'),
      right = new AstTreeNode(null, null, 'right');
      root.addRight(right);
      ...
      expect(right.left).toBe(undefined);
      expect(right.right).toBe(undefined);
    });

    it("Should test isChildOf", ()=>{
      expect(rightLeft.isChildOf(right)).toBe(true);
      expect(rightRight.isChildOf(right)).toBe(true);
      expect(right.isChildOf(rightLeft)).toBe(false);
      expect(right.isChildOf(rightRight)).toBe(false);
      expect(rightLeft.isChildOf(root)).toBe(true);
      expect(rightRight.isChildOf(root)).toBe(true);
      expect(root.isChildOf(rightLeft)).toBe(false);
      expect(right.isChildOf(root)).toBe(true);
      expect(root.isChildOf(right)).toBe(false);

      expect(leftLeft.isChildOf(left)).toBe(true);
      expect(leftRight.isChildOf(left)).toBe(true);
      expect(left.isChildOf(leftLeft)).toBe(false);
      expect(left.isChildOf(leftRight)).toBe(false);
      expect(leftLeft.isChildOf(root)).toBe(true);
      expect(leftRight.isChildOf(root)).toBe(true);
      expect(root.isChildOf(leftLeft)).toBe(false);
      expect(root.isChildOf(leftRight)).toBe(false);
      expect(left.isChildOf(root)).toBe(true);
      expect(root.isChildOf(left)).toBe(false);
    });

    it("Should test popLeft()", ()=>{
      const left = root.popLeft();
      expect(left.isChildOf(root)).toBe(false);
      expect(left.right).toBe(leftRight);
      expect(left.left).toBe(undefined);
      expect(leftLeft.parent).toBe(root);
    });

    it("Should test popRight()", ()=>{
      const right = root.popRight();
      expect(right.isChildOf(root)).toBe(false);
      expect(right.left).toBe(rightLeft);
      expect(right.right).toBe(undefined);
      expect(rightRight.parent).toBe(root);
    });

    it("Should test popChild(child)", ()=>{
      const right2 = root.popChild(right);
      expect(right2.isChildOf(root)).toBe(false);
      expect(right2.left).toBe(rightLeft);
      expect(right2.right).toBe(undefined);
      expect(rightRight.parent).toBe(root);
    });

    it("Should test replaceChild", ()=>{
      const newNode = new AstTreeNode(null, null, 'newNode'),
            newNode2 = new AstTreeNode(null, null, 'newNode2')
      expect(right.replaceChild(newNode, leftLeft)).toBe(false); // leftLeft not a child
      expect(right.replaceChild(newNode, rightLeft)).toBe(true);
      expect(right.left).toBe(newNode);
      expect(newNode.parent).toBe(right);
      expect(rightLeft.parent).toBe(undefined);

      expect(right.replaceChild(newNode2, leftLeft)).toBe(false);
      expect(right.replaceChild(newNode2, rightRight)).toBe(true);
      expect(right.right).toBe(newNode2);
      expect(newNode2.parent).toBe(right);
      expect(rightRight.parent).toBe(undefined);
    });

    it("Should test removeChild", ()=>{
      expect(right.removeChild(leftLeft)).toBe(false);
      expect(right.removeChild(rightLeft)).toBe(true);
      expect(rightLeft.parent).toBe(undefined);
      expect(right.left).toBe(undefined);
      expect(left.removeChild(leftLeft)).toBe(true);
      expect(left.left).toBe(undefined);
      expect(leftLeft.parent).toBe(undefined);
    });
  });
});
...
registerTestSuite("test Grammar AST parser", ()=>{
  describe("test GrammarParser AST generation", ()=>{
    ...
    it("Should throw rule declared twice", ()=>{
      const grammar = `
        start  = term, ";" ;
        term   = 'abc' ;
        term   = 'cdef' ;
...
registerTestSuite("test Parser, AST generation", ()=>{
  describe("Test the CST -> AST generation part of Parser", ()=>{
    ...
    it("Should attach same actions to all",()=>{
      ...
      expect(visited[5]).toBeObj({tp:'terminal',vlu:'id1'});
      expect(visited[6]).toBeObj({tp:'expression',vlu:''});
      expect(visited[7]).toBeObj({tp:'number',vlu:''});
      expect(visited[8]).toBeObj({tp:'terminal',vlu:'123'})
    ...
    it("Should bypass rule",()=>{
      const parser = makeParser({
        source: "var id1=123;",
        flattenRules:['number','identifier'],
        bypassRules:['expression'],
        dropTerminalsOnRules:{varStmt:['var'],assignment:['=',';']},
      });
      const root = parser.prgAstRoot;
      expect(root.left?.left?.left?.right?.type).toBe('number');
    });
    it("Should throw when bypass rule with many children",()=>{
      expect(()=>{
        makeParser({
          source: "var id1=123;",
          bypassRules:['expression', 'number'],
          dropTerminalsOnRules:{varStmt:['var'],assignment:['=',';']},
        });
      }).toThrow(GrammarSyntaxError);
    });
    it("Should bypass all one item rules",()=>{
      const parser = makeParser({
        source: "var id1=123;",
        bypassRules:true,
        dropTerminalsOnRules:{varStmt:['var'],assignment:['=',';']},
    ...
    it("Should delegate to specific to generateFunc",()=>{
      ...
      expect(expression?.type).toBe('expression');
      expect(number?.type).toBe('number');
      expect(identifier?.left?.type).toBe("terminal");
      expect(identifier?.left?.value).toBe("id1");
      expect(number?.left?.type).toBe('terminal');
