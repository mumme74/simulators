...
registerTestSuite("test_Lexer", ()=>{
  ...
  describe("test curTok(), nextTok() and peekTok()", ()=>{
    it("Should move token +1", ()=>{
      const lex = new Lexer({tokenMatches, source});
      expect(lex.curTok()).toBe(lex.tokens[1]); // 0 is a comment
      expect(lex.nextTok()).toBe(lex.tokens[1]);
      expect(lex.nextTok()).toBe(lex.tokens[2]);
      expect(lex.nextTok()).toBe(lex.tokens[3]);
      expect(lex.peekTok()).toBe(lex.tokens[5]);
      expect(lex.peekTok(2)).toBe(lex.tokens[6]);
      expect(lex.peekTok(-1)).toBe(lex.tokens[3]);
      expect(lex.curTok()).toBe(lex.tokens[4]);
...
registerTestSuite("Test parser", ()=>{
  describe("Test GrammarParser", ()=>{
    it("Should scan EBNF grammar", ()=>{
      ...
      expect(grmParser.lexer.tokens.map(t=>t.type)
        ...
        .toContain([
          "string", "equal", "comma", "semicolon",
          "bar", "lbrace", "rbrace", "identifier"
        ]);
