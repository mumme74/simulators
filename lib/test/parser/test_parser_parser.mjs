...
registerTestSuite("Test parser", ()=>{
  describe("Test GrammarParser", ()=>{
    it("Should scan EBNF grammar", ()=>{
      ...
      expect(Object.keys(grmParser.lexer.tokens)).toContain([
        "comment", "string", "equal", "comma", "dot", "semicolon",
        "bar", "lt", "gt", "lparen", "rparen", "lbracket", "rbracket",
        "lbrace", "rbrace", "identifier"
      ]);
      const ebnf = ebnfGrammar.split('\n');
      grmParser.lexer.lines.forEach((l,i)=>{
        console.log('lex',l); console.log('bnf',ebnf[i])
      });
      //throw "stop";
    });
  });
