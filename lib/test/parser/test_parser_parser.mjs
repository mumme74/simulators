...
registerTestSuite("Test parser", ()=>{
  ...
  describe("Test GrammarParser CST", ()=>{
    ...
    const compareTree = (tree, cmp)=>{
      if (tree.t!==cmp.t || tree.s !== cmp.s)
        return false;
      if (tree.c.length !== cmp.c.length)
        return false;
      for (let i = 0; i < tree.c.length; ++i)
        if (compareTree(tree.c[i], cmp.c[i]))
          return false;
      return true;
    }
 ...
 */
    ...
    it("Should throw unclosed terminal", ()=>{
      let obj = {src:`id="A`};
      expect(()=>{throwParse(obj)}).toThrow(GrammarSyntaxError);
      expect(obj.err.message.substr(0,21)).toBe("Expected matching '\"'");
      expect(obj.err.parseInfo).toBeObj({line:1,col:3});
    });
