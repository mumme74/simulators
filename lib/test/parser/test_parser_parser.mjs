...
registerTestSuite("test Parser, CST generation", ()=>{
  describe("test Grammar to CST", ()=>{
    it("Should construct a CST parser from grammar", ()=>{
      ...
      expect(root.children[2]?.children[0]?.tok.parseInfo.line).toBe(2);
      expect(root.children[2]?.type).toBe('factor');
      expect(root.children[2]?.children[0]?.tokString()).toBe('123');
      expect(root.children[2]?.children[1]?.tokString()).toBe(';');
    });
...
registerTestSuite("test Parser, simplelang", ()=>{
  describe("test Constructor", ()=>{
    ...
    /*it("Should parse test program", ()=>{
      ...
      const parser = new Parser({grmParser, source, trace:true});
      const root = parser.prgCstRoot;
