  ...
  describe("test Constructor", ()=>{
    ...
    it("Should construct a CST parser from grammar", ()=>{
      ...
      expect(root.flat().map(n=>n.type)).toBeObj([
       'program', 'repetition',
        'statement', 'concatination',
         'repetition', 'alternation',
          'expression', 'assignment',
           'alternation',
            'logic_or', 'concatination',
             'logic_and', 'concatination',
              'equality', 'concatination',
               'comparison', 'concatination',
                'term', 'alternation', 'concatination',
                 'factor', 'alternation', 'concatination',
                  'unary', 'alternation', 'concatination', 'concatination',
                   'call', 'concatination',
                    'primary', 'alternation',
                     'number', 'concatination',
                      'digit', 'alternation',
                       'terminal', 'concatination',
                        'optional', 'repetition',
                         'digit', 'alternation',
                          'terminal', 'digit',
                         'alternation',
                ...
                'concatination',
               'concatination',
        'grouping', // in statement
         'alternation', 'optional', 'terminal']);
      });

    it("Should construct a CST parser from grammar", ()=>{
      const parser = new Parser(grmParser, `var lang1 = 123;\n`);
      const root = parser.prgCstRoot;
      //console.log(parser.trace())
      expect(root.children.length).toBe(1);
      expect(root.children[0].tokString()).toBe('varlang1=123;');
      expect(root.flat().map(n=>n.type)).toBeObj([
        'program', 'repetition',
         'statement', 'concatination', 'repetition', 'alternation',
          'declaration', 'concatination',
           'terminal', 'concatination', 'grouping', 'alternation',
            'assignment', 'alternation', 'concatination',
             'identifier', 'concatination',
                ...
                'logic_and', 'concatination',
                 'equality', 'concatination',
                  'comparison', 'concatination',
                   ...
                   'concatination',
                  'concatination',
           'concatination', 'grouping', 'alternation', 'optional',
            'terminal']);
    });
  });
